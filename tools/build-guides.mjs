...
// The "Stand" chip used to be a hardcoded month, which silently went stale on
// every page at the same time. Derive it from the guide's own date instead.
const MONTHS_LONG_DE = ["Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember"];
const monthYear = (iso) => {
  const m = /^(\d{4})-(\d{2})/.exec(String(iso || ""));
  return m ? `${MONTHS_LONG_DE[+m[2] - 1]} ${m[1]}` : "";
};
  ...
  <main class="page guide-page">
    ...
    <div class="page-hero">
      ...
      <div class="chip-row">
        ...
        ${g.date ? `<span class="chip">Stand ${esc(monthYear(g.date))}</span>` : ""}
      </div>
